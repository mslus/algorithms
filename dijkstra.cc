...
bool operator<(const node &n1,const node &n2){
	...
	else if(n1.n != n2.n) return n1.n > n2.n;
	else return false;
}
...
vector<int> dijkstra(int start, const vector< vector< pair<int,int> > >  &nb){
	...
	while(!pq.empty()){
		...
		for(vector< pair<int,int> >::const_iterator it = nb[nd.n].begin(); it != nb[nd.n].end(); it++)
			if(!visited[it->first]) pq.push(node(it->first,nd.cost + it->second));						
	}
